...
dir.iterator = function (dirChain, opts) {
  ...
  function getNextEntry (cb) {
    ...
    getSectorBuffer(secIdx, (e, buf) => {
      ...
      let attrByte = buf[entryIdx + S.dirEntry.fields.Attr.offset],
      ...
      if (entryType === S.longDirEntry) {
        ...
        if (firstEntry || long && entry.Chksum === long.sum && entry.Ord === long._rem--) {
          let S_lde_f = S.longDirEntry.fields,
        ...
        if (long && long.name) {
          let pos = entryIdx + S.dirEntry.fields.Name.offset,
        ...
        if (!bestName) {
          ...
          let nam = entry.Name.filename.replace(/ +$/, ''),
        ...
        const _entryBuffer = buf.slice(off.bytes - S.dirEntry.size, off.bytes);
...
function _updateEntry (vol, entry, newStats) {
  ...
  if ('mode' in newStats) {
    entry.Attr.directory = Boolean(newStats.mode & S._I.FDIR);
    entry.Attr.volume_id = !((newStats.mode & S._I.FREG));
    if (vol.opts.modmode === 0111) {
      entry.Attr.archive = Boolean(newStats.mode & S._I.XUSR);
      entry.Attr.system = Boolean(newStats.mode & S._I.XGRP);
      entry.Attr.hidden = Boolean(newStats.mode & S._I.XOTH);
      entry.Attr.readonly = !((newStats.mode & S._I.WUSR));
    } else if (vol.opts.modmode === 07000) {
      entry.Attr.archive = Boolean(newStats.mode & S._I.SVTX);
      entry.Attr.system = Boolean(newStats.mode & S._I.SGID);
      entry.Attr.hidden = Boolean(newStats.mode & S._I.SUID);
      entry.Attr.readonly = !((
...
dir.addFile = function (vol, dirChain, entryInfo, opts, cb) {
  ...
  vol.allocateInFAT(dirChain.firstCluster || 2, (e, fileCluster) => {
    if (e) return cb(e);

    let nameBuf = S.dirEntry.fields.Name.bytesFromValue(mainEntry.Name),
    ...
    dirChain.writeToPosition(entryInfo.target, entriesData, (e) => {
...
dir.findInDirectory = function (vol, dirChain, name, opts, cb) {
  let matchName = name.toUpperCase(),
